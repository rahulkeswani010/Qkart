...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    ...
    if(!addresses.all.length){
      enqueueSnackbar("Please add new address before proceeing",
      {variant:"warning"});
      return false;
    }
    if(!addresses.selected.length){
      enqueueSnackbar("Please select one shipping address before proceed",
      {variant:"warning"});
      return false;
    }
    return true;
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
      ...
      if(e.response){
        ...
        enqueueSnackbar("Could not placed order. Check that the balance is running",{variant:"error"})
      }
    }
  };
